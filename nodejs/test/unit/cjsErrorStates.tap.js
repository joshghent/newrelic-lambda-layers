...
tap.test('CJS Edge Cases', (t) => {
  ...
  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER is missing', (t) => {
    t.throws(() => testFn(), 'No NEW_RELIC_LAMBDA_HANDLER environment variable set.')
    t.end()
  })

  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER is malformed', (t) => {
    t.throws(() => testFn(), 'Improperly formatted handler environment variable: test/unit/fixtures/cjs/handler')
    t.end()
  })

  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER module cannot be resolved',
    (t) => {
  ...
  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER does not export provided function', (t) => {
    t.throws(() => testFn(), `Handler '${handlerMethod}' missing on module '${handlerPath}'`)

    t.end()
  })

  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER export is not a function', (t) => {
    t.throws(() => testFn(), `Handler '${handlerMethod}' from 'test/unit/fixtures/cjs/errors' is not a function`)

    t.end()
  })

  t.test('should throw when NEW_RELIC_LAMBDA_HANDLER throws on import', (t) => {
    t.throws(() => testFn(), `Unable to import module '${handlerPath}${handlerFile}'`)
    t.end()
  })
  t.end()
})
